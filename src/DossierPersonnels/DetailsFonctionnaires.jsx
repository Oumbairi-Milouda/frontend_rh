...
//  const { fonctionnaire, loading, error } = useSelector((state) => state.InformationsReducer);
 ...
 useEffect(() => {
  const fetchFonctionnaire = async () => {
     try {
       setLoading(true);
       const response = await fetch(`http://localhost:3000/informations/${id}`);
       if (!response.ok) {
      ...
      <table className="w-full border-collapse border">
        <tbody>
          ...
          <tr>
            ...
            <td className="border px-4 py-2">{fonctionnaire.stagiaire.user.nom}</td>
          </tr>
          <tr>
            <td className="border px-4 py-2">Prénom :</td>
            <td className="border px-4 py-2">{fonctionnaire.stagiaire.user.prenom}</td>
          </tr>
        </tbody>
      </table>
    </div>
